...
async function sendConfirmationMail(booking) {
  ...
  await sendMail({
    ...
    text: [
      `Hallo ${booking.parentName},`,
      ``,
      `dein Termin für ${booking.studentName} ist bestätigt:`,
      ``,
      `${booking.offerSnapshot?.title}`,
      `Termin: ${formatDate(booking.requestedDate)} um ${booking.requestedTime} Uhr`,
      `Ort: ${locationLabel(booking)}`,
      ``,
      `Bei Fragen erreichst du uns unter ${settings.contactEmail || settings.contactPhone || "den bekannten Kontaktdaten"}.`,
      ``,
      `Viele Grüße`,
      settings.siteName,
    ].join("\n"),
