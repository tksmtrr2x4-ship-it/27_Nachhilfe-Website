...
# Installiert nach ~/.local/bin/, gestartet von launchd alle 6 Stunden und beim Anmelden.
...
if /usr/bin/rsync -a --ignore-existing --include='lernsprung-*.tar.enc' --include='lernsprung-*.tar.enc.sha256' --exclude='*' \
  ...
  [ $bad -eq 0 ] && log "OK, $(ls lernsprung-*.tar.enc 2>/dev/null | wc -l | tr -d ' ') Backups lokal"
else
