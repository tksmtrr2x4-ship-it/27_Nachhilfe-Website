...
export async function recordStripePayment(booking) {
  try {
    ...
    if (err?.code === 11000) return null; // bereits verbucht
    throw err;
  }
}

// Barzahlung für eine oder mehrere Stunden ohne Rechnung. Die Stunden werden
// zuerst atomar reserviert (nur wenn noch abrechenbar), dann wird gebucht –
// so kann dieselbe Stunde weder doppelt bar verbucht noch zusätzlich in eine
// Rechnung übernommen werden.
export async function recordCashLessonPayment({ bookingIds, date, student, counterparty }) {
  ...
  const reserved = await bookings.updateMany(
    { _id: { $in: ids }, invoiceId: null, paymentLedgerEntryId: null },
    { $set: { paymentLedgerEntryId: entryId } }
  );
  if (reserved.modifiedCount !== ids.length) {
    await bookings.updateMany({ paymentLedgerEntryId: entryId }, { $set: { paymentLedgerEntryId: null } });
    ...
    return await insertEntry(
      {
        type: "income",
        date,
        category: "tutoring_cash",
        method: "cash",
        amountCents: list.reduce((s, b) => s + (b.offerSnapshot?.priceCents || 0), 0),
        description: `Barzahlung Nachhilfe ${student.name}: ${list.length} ${list.length === 1 ? "Stunde" : "Stunden"}${dates.length ? ` (${dates.map((d) => d.split("-").reverse().join(".")).join(", ")})` : ""}`,
