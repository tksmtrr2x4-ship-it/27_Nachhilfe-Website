import crypto from "crypto";
import { getDb } from "@/lib/mongo";

const SETTINGS_ID = "site";

const DEFAULT_SETTINGS = {
  siteName: "Lernsprung",
  slogan: "Nachhilfe, die wirklich ankommt.",
  subline: "Nachhilfe ab Klasse 8 – persönlich, verständlich, mit messbarem Erfolg.",
  aboutTitle: "Warum Lernsprung?",
  aboutText:
    "Ich weiß noch genau, wie es sich anfühlt, im Unterricht den Anschluss zu verlieren. Deshalb erkläre ich dir Mathe, Physik, Biologie oder Wirtschaft so, dass es wirklich sitzt – auf Augenhöhe, ohne Zeitdruck, ohne lange Vertragsbindung.",
  aboutBullets: [
    "Nachhilfe von Schüler zu Schüler-Niveau – kein Frontalunterricht wie früher",
    "Individuell auf dein Fach, deine Klasse und deine Lücken abgestimmt",
    "Persönlich in Villingen-Schwenningen oder online",
  ],
  contactEmail: "",
  contactPhone: "",
  minClass: 8,
  maxClass: 13,
  // Adresse der Lehrkraft, wird Kund:innen angezeigt, die "bei der Lehrkraft"
  // als Unterrichtsort für eine Einzelstunde wählen.
  tutorAddress: "",
  // Freier Text, den die Lehrkraft selbst festlegt (keine feste Uhrzeit-
  // Einschränkung im Buchungsformular – jeder Tag/jede Uhrzeit ist wählbar,
  // die Bestätigung erfolgt ohnehin manuell). Wird auf der Buchungsseite
  // angezeigt, wenn ausgefüllt.
  openingHoursText: "",
  // Shop-Schalter: bei false werden keine neuen Buchungen mehr angenommen.
  shopOpen: true,
  // Freitext, der Kund:innen angezeigt wird, während der Shop geschlossen ist.
  shopClosedMessage: "",
  // Optionales Datum (YYYY-MM-DD), ab dem der Shop wieder öffnet.
  shopReopensAt: "",
  // Rechtsform-Hinweis für Preisangaben/Impressum: true = Kleinunternehmer
  // nach §19 UStG (keine USt ausgewiesen), false = regelbesteuert mit USt-ID.
  kleinunternehmer: true,
  ustId: "",
};

async function offersCol() {
  return (await getDb()).collection("offers");
}
async function bookingsCol() {
  return (await getDb()).collection("bookings");
}
async function settingsCol() {
  return (await getDb()).collection("settings");
}
async function countersCol() {
  return (await getDb()).collection("counters");
}

// Kurze, fortlaufende Buchungsnummer je Kalenderjahr, z.B. "2026-014".
// Atomarer Zähler in eigener Collection, damit parallele Buchungen nie
// dieselbe Nummer bekommen.
async function nextBookingNumber() {
  const year = new Date().getFullYear();
  const col = await countersCol();
  const updated = await col.findOneAndUpdate(
    { _id: `booking-${year}` },
    { $inc: { seq: 1 } },
    { upsert: true, returnDocument: "after" }
  );
  const seq = updated.seq;
  return `${year}-${String(seq).padStart(3, "0")}`;
}

// Drops undefined keys so a partial patch can't overwrite existing fields
// with undefined (which MongoDB's $set would otherwise store as null-ish).
function stripUndefined(obj) {
  return Object.fromEntries(
    Object.entries(obj).filter(([, v]) => v !== undefined)
  );
}

// ---- Settings (single document with a fixed _id) ----

export async function getSettings() {
  const col = await settingsCol();
  const doc = await col.findOne({ _id: SETTINGS_ID });
  const { _id, ...stored } = doc || {};
  return { ...DEFAULT_SETTINGS, ...stored };
}

export async function updateSettings(patch) {
  const col = await settingsCol();
  await col.updateOne(
    { _id: SETTINGS_ID },
    { $set: stripUndefined(patch) },
    { upsert: true }
  );
  return getSettings();
}

// ---- Offers ----

export async function listOffers({ onlyActive = false } = {}) {
  const col = await offersCol();
  const query = onlyActive ? { active: true } : {};
  return col.find(query).sort({ order: 1 }).toArray();
}

export async function getOffer(id) {
  const col = await offersCol();
  return col.findOne({ _id: id });
}

export async function createOffer(payload) {
  const col = await offersCol();
  const offer = {
    _id: crypto.randomUUID(),
    order: await col.countDocuments(),
    type: "package", // "package" (Kursabo etc.) oder "session" (Einzelstunde 45/90 Min.)
    durationMinutes: null,
    title: "",
    subject: "",
    durationLabel: "",
    description: "",
    features: [],
    priceCents: 0,
    // Streichpreis (Vergleichspreis) für den Rabatt-Badge – wird von der
    // Lehrkraft selbst im Admin-Bereich festgelegt, nicht automatisch
    // berechnet.
    listPriceCents: null,
    active: true,
    // Strukturierte Felder für Pakete, nur für die Dauer-/Umfangs-Anzeige
    // (z.B. "8 Einheiten à 45 Min. · Laufzeit 4 Wochen").
    sessionCount: null,
    sessionMinutes: null,
    weeks: null,
    mode: "both", // "online" | "presence" | "both"
    catchmentAreaText: "",
    cancellationText: "",
    validityText: "",
    // Steuert Checkbox 3 im Buchungsformular (§ 356 Abs. 4 BGB) bei Paketen:
    // true = Leistungsbeginn liegt typischerweise innerhalb von 14 Tagen
    // (z.B. "Last-Minute-Boarding"). Bei Einzelstunden wird das stattdessen
    // direkt aus dem gewählten Terminwunsch berechnet.
    earlyStartPossible: false,
    // Klassenstufen-Bereich, für den dieses konkrete Angebot gilt (frei pro
    // Angebot einstellbar, unabhängig vom allgemeinen settings.minClass/
    // maxClass). null bei beiden = keine Einschränkung/Angabe, das Angebot
    // gilt dann für jede Klasse. Steuert auch die Klassen-Vorauswahl auf
    // /angebote.
    minClass: null,
    maxClass: null,
    ...payload,
  };
  await col.insertOne(offer);
  return offer;
}

export async function updateOffer(id, patch) {
  const col = await offersCol();
  const updated = await col.findOneAndUpdate(
    { _id: id },
    { $set: stripUndefined(patch) },
    { returnDocument: "after" }
  );
  return updated || null;
}

export async function deleteOffer(id) {
  const col = await offersCol();
  const res = await col.deleteOne({ _id: id });
  return res.deletedCount > 0;
}

// ---- Testimonials (Rückmeldungen) ----

async function testimonialsCol() {
  return (await getDb()).collection("testimonials");
}

export async function listTestimonials({ onlyActive = false } = {}) {
  const col = await testimonialsCol();
  const query = onlyActive ? { active: true } : {};
  return col.find(query).sort({ order: 1 }).toArray();
}

export async function createTestimonial(payload) {
  const col = await testimonialsCol();
  const testimonial = {
    _id: crypto.randomUUID(),
    order: await col.countDocuments(),
    name: "",
    role: "", // z.B. "Mutter von Max, Klasse 9"
    text: "",
    active: true,
    ...payload,
  };
  await col.insertOne(testimonial);
  return testimonial;
}

export async function updateTestimonial(id, patch) {
  const col = await testimonialsCol();
  const updated = await col.findOneAndUpdate(
    { _id: id },
    { $set: stripUndefined(patch) },
    { returnDocument: "after" }
  );
  return updated || null;
}

export async function deleteTestimonial(id) {
  const col = await testimonialsCol();
  const res = await col.deleteOne({ _id: id });
  return res.deletedCount > 0;
}

// ---- Bookings ----

export async function listBookings() {
  const col = await bookingsCol();
  return col.find({}).sort({ createdAt: -1 }).toArray();
}

export async function getBooking(id) {
  const col = await bookingsCol();
  return col.findOne({ _id: id });
}

// Für die öffentliche Meeting-Seite (/meeting/[token]) – bewusst keine
// Admin-Auth nötig, der lange Zufallstoken selbst ist die Zugangskontrolle.
export async function getBookingByMeetingToken(token) {
  if (!token) return null;
  const col = await bookingsCol();
  return col.findOne({ meetingToken: token });
}

export async function createBooking(payload) {
  const col = await bookingsCol();
  const bookingNumber = await nextBookingNumber();
  const booking = {
    _id: crypto.randomUUID(),
    bookingNumber,
    offerId: "",
    offerSnapshot: null,
    studentName: "",
    studentClass: "",
    // Anzeigename inkl. Kursniveau, z. B. "Mathematik (Leistungsfach)";
    // subjectName/courseLevel getrennt (siehe lib/subjectRules.js).
    subject: "",
    subjectName: "",
    courseLevel: "",
    parentName: "",
    parentEmail: "",
    parentPhone: "",
    notes: "",
    status: "pending",
    // Einwilligung nach § 107 BGB: Erziehungsberechtigte:r bestätigt,
    // stellvertretend für die minderjährige Schülerin/den Schüler den
    // Vertrag abzuschließen.
    guardianConsent: false,
    // Beweisbares Protokoll der im Buchungsformular gesetzten Checkboxen:
    // { [key]: { text, checkedAt } } – exakt angezeigter Wortlaut + Zeitstempel.
    consents: {},
    // Nur bei Einzelstunden-Buchungen (Terminanfrage).
    requestedDate: null,
    requestedTime: null,
    locationType: null, // "tutor" | "student" | "online"
    locationAddress: "",
    // Nur bei Online-Einzelstunden, sobald im Admin-Bereich bestätigt: langer
    // Zufallswert, der zugleich als URL-Bestandteil (/meeting/<token>) und als
    // Jitsi-Raumname dient. Macht den Meeting-Link praktisch unerratbar, ohne
    // ein separates Login-System für Kund:innen zu brauchen.
    meetingToken: null,
    createdAt: new Date().toISOString(),
    paidAt: null,
    confirmedAt: null,
    // Verhindert doppelten Versand der Bestellbestätigung (§ 312f Abs. 2 BGB),
    // z.B. bei erneutem Bestätigen nach einem Statuswechsel.
    confirmationEmailSentAt: null,
    // Verwaltung (Admin → Schüler & Buchhaltung): Schülerprofil, Herkunft
    // ("web" = Online-Buchung, "admin" = selbst eingetragen), Barzahlung und
    // Stundenprotokoll.
    studentId: null,
    source: "web",
    paymentLedgerEntryId: null,
    paymentMethod: null,
    // { at, note }: vor Einführung des Journals außerhalb davon abgerechnet.
    settledExternally: null,
    lessonNotes: "",
    ...payload,
  };
  await col.insertOne(booking);
  return booking;
}

export async function updateBooking(id, patch) {
  const col = await bookingsCol();
  const updated = await col.findOneAndUpdate(
    { _id: id },
    { $set: stripUndefined(patch) },
    { returnDocument: "after" }
  );
  return updated || null;
}

export async function deleteBooking(id) {
  const col = await bookingsCol();
  const res = await col.deleteOne({ _id: id });
  return res.deletedCount > 0;
}
