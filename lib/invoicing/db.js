...
export async function ensureInvoiceIndexes() {
  ...
  // Frische Datenbank ohne invoices-Collection: nichts zu migrieren.
  const existing = await invoices.indexes().catch((err) => {
...
// storniert), noch keiner Rechnung
...
export async function listUnbilledSessions(email) {
  ...
  const all = await col
    ...
    .toArray();
  // Regel (inkl. "bar bezahlt → nicht mehr abrechenbar") in lib/lessons/rules.js.
  return all.filter((b) => String(b.parentEmail || "").toLowerCase() === emailLower && isBillableSession(b, today));
}
...
// Positionsvorschlag aus einer bestätigten Einzelstunde oder einem Paket (im
// Entwurf weiterhin frei editierbar). Pakete haben keinen Termin: Als
// Leistungsdatum dient der Tag der Bestätigung.
export function lineFromBooking(booking) {
  const snap = booking.offerSnapshot || {};
  if (snap.type === "package") {
    return {
      date: booking.confirmedAt
        ? new Date(booking.confirmedAt).toLocaleDateString("sv-SE", { timeZone: "Europe/Berlin" })
        : todayIsoBerlin(),
      description: `Nachhilfe-Paket „${snap.title || ""}“ ${booking.subject ? `(${booking.subject})` : ""}`.trim(),
      minutes: null,
      quantity: 1,
