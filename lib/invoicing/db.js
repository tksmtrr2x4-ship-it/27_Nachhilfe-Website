...
export async function ensureInvoiceIndexes() {
  ...
  // zusätzlich zum atomaren Zähler. Sparse: Entwürfe haben keine Nummer.
  await invoices.createIndex({ number: 1 }, { unique: true, sparse: true });
...
export async function findOrCreateCustomerFromBooking(booking) {
  ...
  if (existing) {
    ...
    if (!existing.eInvoiceConsent?.given && consentFromBooking) {
      return updateCustomer(existing._id, { eInvoiceConsent: consentFromBooking });
    }
    return existing;
  }
