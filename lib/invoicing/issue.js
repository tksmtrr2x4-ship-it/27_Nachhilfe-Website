...
export async function issueInvoice(id) {
  ...
  // Bereits bar bezahlte Stunden (Buchhaltung → Barzahlung) dürfen nicht
  // zusätzlich in Rechnung gestellt werden.
  const draftBookings = await getBookingsByIds(draft.bookingIds || []);
  const paidCash = draftBookings.filter((b) => b.paymentLedgerEntryId);
  if (paidCash.length > 0) {
    problems.push(`${paidCash.length === 1 ? "Eine Stunde ist" : `${paidCash.length} Stunden sind`} bereits bar bezahlt und verbucht – bitte aus der Rechnung entfernen.`);
  }
