...
import { recordInvoicePayment, reverseInvoicePayment } from "@/lib/bookkeeping/db";
import {
  getBookingsByIds,
  getInvoice,
  getCustomer,
...
export async function issueInvoice(id) {
  ...
  // Bereits ohne Rechnung bezahlte oder vor Einführung des Journals
  // abgerechnete Stunden dürfen nicht zusätzlich in Rechnung gestellt werden.
  const draftBookings = await getBookingsByIds(draft.bookingIds || []);
  const settled = draftBookings.filter((b) => b.paymentLedgerEntryId || b.settledExternally);
  if (settled.length > 0) {
    problems.push(`${settled.length === 1 ? "Eine Stunde ist" : `${settled.length} Stunden sind`} bereits ohne Rechnung bezahlt bzw. früher abgerechnet – bitte aus der Rechnung entfernen.`);
  }
...
const PAYMENT_METHOD_KEYS = ["bank", "cash", "card"];

// Zahlungseingang: Status "bezahlt" UND Einnahme im Buchhaltungs-Journal
// (Zuflussprinzip: Datum = Zahlungsdatum). Scheitert die Buchung, wird der
// Status zurückgesetzt, damit Rechnung und Journal nie auseinanderlaufen.
export async function markInvoicePaid(id, paidAt, method = "bank") {
  const date = paidAt || todayIsoBerlin();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) throw new InvoiceError("Ungültiges Zahlungsdatum.");
  if (date > todayIsoBerlin()) throw new InvoiceError("Das Zahlungsdatum darf nicht in der Zukunft liegen.");
  if (!PAYMENT_METHOD_KEYS.includes(method)) throw new InvoiceError("Ungültige Zahlungsart.");
  const updated = await transitionInvoice(id, ["issued", "sent"], { status: "paid", paidAt: date, paymentMethod: method });
  if (!updated) throw new InvoiceError("Nur ausgestellte/versendete Rechnungen können als bezahlt markiert werden.", { status: 409 });
  try {
    await recordInvoicePayment(updated, { method, date });
  } catch (err) {
    await transitionInvoice(id, ["paid"], { status: updated.sentAt ? "sent" : "issued", paidAt: null, paymentMethod: null });
    throw err;
  }
  return updated;
}
...
export async function markInvoiceUnpaid(id) {
  ...
  const reverted = await transitionInvoice(id, ["paid"], { status: invoice.sentAt ? "sent" : "issued", paidAt: null, paymentMethod: null });
  // Buchung nicht löschen, sondern gegenbuchen (GoBD).
  if (reverted) await reverseInvoicePayment(invoice);
  return reverted;
}
