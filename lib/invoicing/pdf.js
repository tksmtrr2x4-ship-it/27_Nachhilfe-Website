...
function drawPageChrome(doc, { seller, invoice, isDraft }) {
  ...
  // Steuerangaben sind optional: Ohne vergebene Steuernummer/USt-IdNr bleibt
  // die Zeile einfach kürzer – kein Platzhalter, keine leere Beschriftung.
  // § 14 Abs. 4 Nr. 2 UStG verlangt Steuernummer ODER USt-IdNr nur, soweit
  // eine vergeben ist; die persönliche Steuer-Identifikationsnummer nach
  // § 139b AO ist ausdrücklich KEIN zulässiger Ersatz.
  const footerParts = [seller.email, seller.phone];
  if (seller.taxNumber) footerParts.push(`Steuernummer: ${seller.taxNumber}`);
  if (seller.vatId) footerParts.push(`USt-IdNr.: ${seller.vatId}`);
  doc.text(footerParts.join(" · "), LEFT, footY + 10, { lineBreak: false });
  void invoice;
