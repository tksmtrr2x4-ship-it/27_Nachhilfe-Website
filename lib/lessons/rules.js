...
// Einführung des Buchhaltungs-Journals. Nur Stunden VOR diesem Tag dürfen als
// "vor Einführung abgerechnet" markiert werden (Bezahlung bereits in einer
// früheren EÜR oder anderen Aufzeichnung erfasst) – alles danach läuft über
// Rechnung oder "Als bezahlt verbuchen", damit keine Einnahme am Journal
// vorbeigeht.
export const JOURNAL_START_DATE = "2026-09-16";

export function normalizeSettleInput(body) {
  const note = String(body?.note || "").trim().slice(0, 300);
  const problems = [];
  if (!note) problems.push("Bitte kurz festhalten, wo die Bezahlung erfasst ist (z. B. „bar bezahlt 2025, in EÜR 2025 enthalten“).");
  return { data: { note }, problems };
}

export function isBeforeJournalStart(lesson) {
  return Boolean(lesson?.requestedDate) && lesson.requestedDate < JOURNAL_START_DATE;
}
...
export function isLessonLocked(lesson) {
  return Boolean(lesson?.invoiceId || lesson?.paymentLedgerEntryId || lesson?.settledExternally);
}
...
// Rechnung noch ohne Rechnung bezahlt noch vor Einführung des Journals
// abgerechnet noch online bezahlt, nicht ausgefallen, und
...
export function isBillableSession(b, today) {
  ...
  if (b.invoiceId || b.paymentLedgerEntryId || b.settledExternally) return false;
