...
export function normalizeOfferInput(body, { partial } = { partial: false }) {
  ...
  // Streichpreis für den Rabatt-Badge – wird von der Lehrkraft selbst
  // festgelegt (null = kein Badge).
  if (has("listPriceCents")) {
    out.listPriceCents = Number.isFinite(body.listPriceCents) ? body.listPriceCents : null;
  }
  ...
  // Strukturierte Paket-Felder, nur für die Dauer-/Umfangs-Anzeige.
  ...
  if (has("earlyStartPossible")) out.earlyStartPossible = Boolean(body.earlyStartPossible);
  // Klassenstufen-Bereich, für den das Angebot gilt – frei pro Angebot,
  // leer/0 bedeutet keine Einschränkung an dieser Grenze.
  const toClassNumber = (v) =>
    Number.isFinite(Number(v)) && v !== null && v !== "" ? Number(v) : null;
  if (has("minClass")) out.minClass = toClassNumber(body.minClass);
  if (has("maxClass")) out.maxClass = toClassNumber(body.maxClass);
