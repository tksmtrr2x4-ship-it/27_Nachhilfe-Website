...
export async function sendOrderConfirmationEmail(booking) {
  ...
  const paymentMethod = !isSession
    ...
    : paidOnlineSession && alreadyPaid
      ? "Kartenzahlung über Stripe (bereits bezahlt)"
      : paidOnlineSession
        ? "Kartenzahlung über Stripe (über den Link unten zu bezahlen)"
  ...
  const lines = [
    ...
    paidOnlineSession && !alreadyPaid
      ...
      : meetingUrl
        ...
        : null,
    paidOnlineSession && !alreadyPaid
      ? `(Auf dieser Seite bezahlst du die Stunde; direkt danach wird der Video-Unterricht freigeschaltet. Der Link ist persönlich – bitte nicht weitergeben.)`
      : meetingUrl
