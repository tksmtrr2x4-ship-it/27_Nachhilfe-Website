...
export async function sendOrderConfirmationEmail(booking) {
  ...
  const meetingUrl =
    isSession && booking.locationType === "online" && booking.meetingToken
      ...
      : null;
  // Kostenpflichtige Online-Einzelstunde: bezahlt wird über den Link zur
  // Meeting-Seite (Zahlungs-Gate vor dem Video). 0,00-€-Termine und
  // Vor-Ort-Stunden bleiben "individuelle Absprache".
  ...
  const paymentMethod = !isSession
    ...
    : paidOnlineSession && alreadyPaid
      ? "Kartenzahlung über Stripe (bereits bezahlt)"
      : paidOnlineSession
        ? "Nach Wahl: Kartenzahlung über Stripe oder Zahlung per Rechnung (Auswahl über den Link unten)"
  ...
  const lines = [
    ...
    paidOnlineSession && !alreadyPaid
      ...
      : meetingUrl
        ...
        : null,
    paidOnlineSession && !alreadyPaid
      ? `(Auf dieser Seite wählst du Kartenzahlung oder Zahlung per Rechnung; direkt danach wird der Video-Unterricht freigeschaltet. Der Link ist persönlich – bitte nicht weitergeben.)`
      : meetingUrl
