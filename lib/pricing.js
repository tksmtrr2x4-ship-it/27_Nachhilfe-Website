// Zentrale Preislogik: EIN Basispreis, aus dem sich jeder Paketpreis
// nachvollziehbar ableitet. So können Streichpreis/Rabatt nie wieder
// widersprüchlich zum tatsächlichen Angebotsinhalt werden (vgl. §5 UWG:
// Rabatte müssen einem echten, nachvollziehbaren Vergleichspreis entsprechen).
export const BASE_PRICE_CENTS_PER_45MIN = 3000; // 30 €

// Rechnerischer Vergleichspreis (Streichpreis) einer Leistung mit
// `sessionCount` Einheiten à `sessionMinutes` Minuten, linear zum
// Basispreis pro 45 Minuten.
export function computeListPriceCents(sessionCount, sessionMinutes) {
  if (!sessionCount || !sessionMinutes) return null;
  const unitsOf45 = sessionMinutes / 45;
  return Math.round(sessionCount * unitsOf45 * BASE_PRICE_CENTS_PER_45MIN);
}
