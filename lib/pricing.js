...
// Menschenlesbare Klassenstufen-Angabe für ein Angebot (offer.minClass/
// maxClass, beide optional und unabhängig voneinander setzbar).
export function formatClassRange(offer) {
  const { minClass, maxClass } = offer;
  if (minClass && maxClass) {
    return minClass === maxClass ? `Klasse ${minClass}` : `Klasse ${minClass}–${maxClass}`;
  }
  if (minClass) return `ab Klasse ${minClass}`;
  if (maxClass) return `bis Klasse ${maxClass}`;
  return "";
}

// Ob ein Angebot für eine gewählte Klassenstufe gilt. Angebote ohne
// Klassen-Angabe (weder minClass noch maxClass gesetzt) gelten für jede
// Klasse.
export function offerMatchesClass(offer, selectedClass) {
  if (!selectedClass) return true;
  if (offer.minClass && selectedClass < offer.minClass) return false;
  if (offer.maxClass && selectedClass > offer.maxClass) return false;
  return true;
}
