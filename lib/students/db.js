...
function summarizeLessons(lessons, today) {
  const stats = { held: 0, missed: 0, upcoming: 0, heldMinutes: 0, billable: 0, billableCents: 0, invoiced: 0, paidDirect: 0, settledBefore: 0 };
  for (const l of lessons) {
    ...
    if (l.paymentLedgerEntryId) stats.paidDirect++;
    if (l.settledExternally) stats.settledBefore++;
...
export async function listStudentsWithStats() {
  const [students, lessons] = await Promise.all([
    ...
    (await bookingsCol())
      ...
      .project({ studentId: 1, requestedDate: 1, heldStatus: 1, invoiceId: 1, paymentLedgerEntryId: 1, settledExternally: 1, status: 1, offerSnapshot: 1 })
      .toArray(),
  ]);
